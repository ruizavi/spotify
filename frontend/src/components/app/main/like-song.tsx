...
const LikeSong = () => {
  const { setPlay, setSong, setQueue } = useControl();
  const { data, isLoading } = useLibrary<{
    song: Array<{ like_id: number; entity_id: number; user_id: number }>;
  }>("song");
  const user = useUser((s) => s.user);

  const handleClick = (song: Song, location: "collection/tracks") => {
    setSong(song, location);
    setPlay();

    if (data?.song) {
      setQueue(data?.song.map((s: like_entity) => s.like_id));
    }
  };
...
const SongRow = ({
  ...
  id: number;
  index: number;
  handleClick: (data: Song, location: "collection/tracks") => void;
}) => {
  const token = useUser((s) => s.token);
  const { song } = useControl();
...
const PlaySong = ({
  ...
  const { song, setPlay, isPlay } = useControl();
    ...
    >
      {isHover ? (
        song?.id === data.id ? (
          ...
          <Play
            className="size-4 fill-white"
            onClick={() => handleClick(data, "collection/tracks")}
          />
        )
      ) : (
        index + 1
